...
4. **Ambiente de execução:** o Everton já tem Python instalado na máquina (confirmado: Python 3.14.3 disponível) e sabe (ou vai aprender, com ajuda do README) a rodar `pip install -r requirements.txt` uma única vez. Para converter arquivos, o uso pretendido é dar duplo clique direto em `converter.py` — a associação padrão do Windows com `.py` abre um terminal e roda o script, mesmo comportamento que o Everton já usa em outros scripts seus (ex. `mineracao_emails.py`). Rodar `python nome_do_script.py` num terminal já aberto continua funcionando como alternativa. Continua fora do escopo desta v1 criar um instalador `.exe` ou um atalho (`.lnk`) dedicado — não é necessário, já que o duplo clique no próprio arquivo `.py` atende a necessidade.
